    ...
    return (
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm md:p-7">
            <div className="flex items-start gap-3 border-b border-slate-100 pb-5">
                <span className="material-symbols-outlined flex size-10 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary">add_link</span>
                <div>
                    <h2 className="text-lg font-bold text-slate-950">Dados da cobrança</h2>
                    <p className="mt-1 text-sm leading-5 text-slate-500">O cliente verá a descrição e o valor antes de pagar.</p>
                </div>
            </div>

            <form className="mt-6 space-y-5" noValidate onSubmit={onSubmit}>
                <div>
                    <label className="mb-2 block text-sm font-bold text-slate-800" htmlFor="billing-description">Descrição</label>
                    <textarea
                        ...
                        value={description}
                    />
                    <p className="mt-1.5 text-right font-public text-xs text-slate-500">{description.length}/500</p>
                </div>

                <div className="grid gap-5 sm:grid-cols-2">
                    <div>
                        <label className="mb-2 block text-sm font-bold text-slate-800" htmlFor="billing-amount">Valor da cobrança</label>
                        <div className="relative">
                            <span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 font-bold text-slate-500">R$</span>
                            <input
                    ...
                    <div>
                        <label className="mb-2 block text-sm font-bold text-slate-800" htmlFor="billing-expiration">
                            Data de vencimento <span className="font-normal text-slate-500">(opcional)</span>
                        </label>
                        <input
                            ...
                            onChange={(event) => onExpirationChange(event.currentTarget.value)}
                            type="datetime-local"
                            value={expiresAt}
                        />
                    </div>
                ...
                <div className="rounded-lg border border-blue-100 bg-blue-50/70 p-4 text-sm leading-6 text-blue-950">
                    <div className="flex gap-3">
                        ...
                        <p>Este link não reserva estoque nem cria pedido ou frete. Ele serve apenas para uma cobrança avulsa.</p>
                    </div>
                </div>
